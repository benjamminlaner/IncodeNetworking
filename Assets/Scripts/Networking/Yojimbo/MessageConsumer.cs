...
namespace Prometheus.Game
{
    ...
    public class MessageConsumer
    {
        ...
        private void UpdateState(GameState gameState, PlayerUpdateSystem playerUpdateSystem)
        {
            ...
            {
                if (playerUpdateSystem.playerNetworkDictionary == null)
                {
                    playerUpdateSystem.playerNetworkDictionary = new Dictionary<ulong, NetworkPlayerState>();
                }
                // Create update entities, these entities are just input entities
                // Namely disposable one time entities.
                foreach (var player in gameState.Players)
                {
                    if (!playerUpdateSystem.playerNetworkDictionary.ContainsKey(player.playerId))
                    {
                        playerUpdateSystem.playerNetworkDictionary.Add(player.playerId, new NetworkPlayerState());
                    }
